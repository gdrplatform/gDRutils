...
test_that("gDRutils:::.validate_polymapped_identifiers works as expected", {
  # Set up.
  nrow <- 5
  exp_one_ids <- get_expect_one_identifiers()
  nids <- length(exp_one_ids)
  ...
  # All singletons.
  obs <- gDRutils:::.validate_polymapped_identifiers(df, exp_one_ids, id_map)
  expect_equal(obs, id_map)

  # Some polymappings.
  some_poly_map <- id_map
  some_poly_map[[1]] <- c(some_poly_map[[1]], "extra_item") 
  obs <- gDRutils:::.validate_polymapped_identifiers(df, exp_one_ids, id_map = some_poly_map)
  expect_equal(obs, id_map)
  ...
  all_poly_map <- lapply(seq_along(all_poly_map), function(x) c(all_poly_map[[x]], "extra_item"))
  names(all_poly_map) <- names(id_map)
  obs <- gDRutils:::.validate_polymapped_identifiers(df, exp_one_ids, id_map = all_poly_map)
  expect_equal(obs, id_map)
})
