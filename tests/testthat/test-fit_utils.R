...
test_that("apply_fit_to_se iterates over all (drug x cell line x normalization_type) triplets", {
  ...
  metrics_df <- convert_se_assay_to_dt(result_se, "Metrics",
...
test_that("constant fit derives the xc50 sign from the mean, matching logisticFit", {
  # Same contract as :.set_mean_params(): sign follows the mean normalized
...
#### Parity with logisticFit ####

# fit_drug_response_metrics() reimplements the fit that logisticFit()
...
.logistic_fit_reference <- function(conc, x, x_std = NA, norm_type = "RV", x_0 = 1) {
  # Priors and bounds live in the unexported :.applyLogisticFit() for RV and
  ...
  logisticFit(conc, x, x_std, priors = priors, lower = lower, x_0 = x_0)
}
