test_that("split_SE_components splits the correct columns", {
  ...
  expect_true(all(c("Gnumber", "DrugName", "Replicate", "drug_moa") %in% colnames(md$treatment_md)))
  ...
  md3 <- split_SE_components(test_df, nested_keys = c("Replicate"), combine_on = 2L)
  ...
  expect_true(all(c("Gnumber", "DrugName", "Replicate") %in% colnames(md4$treatment_md)))
  ...
  expect_equal(names(md$treatment_md), c("Gnumber", "DrugName", "drug_moa", "Duration", "Replicate"))
  ...
  expect_equal(names(md5$treatment_md), c(new_identifier_name, "DrugName", "drug_moa", "Duration", "Replicate"))
})
